        ...
        style={{
          boxShadow: scrolled || mobileMenuOpen
            ? (theme === 'dark'
                ...
                : '0 12px 40px 0 rgba(0, 0, 0, 0.08), inset 0 1px 0 0 rgba(255, 255, 255, 0.8)')
            : (theme === 'dark'
                ? '0 4px 20px 0 rgba(0, 0, 0, 0.05), inset 0 1px 0 0 rgba(255, 255, 255, 0.12)'
                : '0 4px 20px 0 rgba(0, 0, 0, 0.02), inset 0 1px 0 0 rgba(255, 255, 255, 0.5)'),
          border: scrolled || mobileMenuOpen
            ? (theme === 'dark' ? '1px solid rgba(255, 255, 255, 0.16)' : '1px solid rgba(0, 0, 0, 0.08)')
          ...
          <div className="hidden md:flex items-center gap-3">
            ...
            */}
            {!hideAuthButton && (user ? (
              <Link
                to="/dashboard"
                className="flex items-center gap-2 px-4 py-2 bg-black/5 dark:bg-white/10 border border-black/10 dark:border-white/20 rounded-xl hover:bg-black/10 dark:hover:bg-white/20 transition-all duration-300"
              >
          ...
          <div className="md:hidden pb-4 border-t border-black/10 dark:border-white/10 mt-2 pt-4 bg-transparent">
            <div className="flex flex-col space-y-1">
              ...
              {!hideNavItems && resourceLinks.map((link, i) =>
                link.to ? (
                  <Link key={i} to={link.to} className="px-4 py-3 text-gray-500 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white hover:bg-black/5 dark:hover:bg-white/10 rounded-lg transition-all duration-200" onClick={() => setMobileMenuOpen(false)}>
                    {link.label}
                  </Link>
                ) : (
                  <a key={i} href={link.href} className="px-4 py-3 text-gray-500 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white hover:bg-black/5 dark:hover:bg-white/10 rounded-lg transition-all duration-200" onClick={() => setMobileMenuOpen(false)}>
                    {link.label}
                  </a>
                )
              )}
              ...
              {!hideAuthButton && (
                <div className="pt-3 mt-2 border-t border-black/10 dark:border-white/10 space-y-2 px-2">
                  {user ? (
                    <Link
                      to="/dashboard"
                      className="flex items-center gap-3 p-3 bg-black/5 dark:bg-white/10 rounded-xl border border-black/10 dark:border-white/20"
                    ...
                    >
                      ...
                      <div>
                        <div className="text-gray-950 dark:text-white font-semibold">{profile?.company_name || 'Dashboard'}</div>
                        <div className="text-xs text-gray-500 dark:text-gray-400">Go to Dashboard →</div>
                      </div>
                    </Link>
                  ) : (
                    <>
