...
const MatchesSection: React.FC<MatchesSectionProps> = ({
  ...
  const confirmRiskAnalysis = async () => {
    ...
    setLoadingRequests(prev => new Set([...prev, pendingOpportunityId]));
    ...
    try {
      ...
      setRequestedAnalyses(prev => new Set([...prev, pendingOpportunityId]));
      toast.success('Risk analysis requested successfully');
    } catch (error: any) {
      console.log('Error requesting risk analysis:', error);
      
    } finally {
      setLoadingRequests(prev => {
        const newSet = new Set(prev);
        newSet.delete(pendingOpportunityId);
        return newSet;
      });
      setIsModalOpen(false);
                        ...
                        <div className="flex flex-wrap gap-2">
                          {match.opportunities?.sponsorship_brochure_url && (
                            <a
                              ...
                              className="px-3 py-1.5 bg-gray-100 text-gray-700 rounded-lg hover:bg-gray-200 flex items-center text-sm transition-transform transform hover:scale-105"
